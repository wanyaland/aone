jQuery(document).ready(function($) {
    ...
    jQuery('form i.cross-search-q').on('click', function() {
        ...
        jQuery.ajax({
            ...
            url: ajax_search_term_object.ajaxurl,
            data: {
                'action': 'africaone_suggested_search',
                'query': qString,
            },
            success: function(data) {
                if (data) {
    ...
    var inputCityField = jQuery('#city_id');
    var inputFeatureField = jQuery('#feature_id');
    var inputCatField = jQuery('#category_id');
    var inputListingField = jQuery('#listing_id');
    ...
    jQuery(document).on('click', '#input-dropdown ul li', function(event) {
        ...
        var key_type = jQuery(this).data('key_type');
        var feature_id = jQuery(this).data('tasgid');
        var category_id = jQuery(this).data('catsid');
        var listing_id = jQuery(this).data('titlesid');
        var moreVal = jQuery(this).data('moreval');
        inputField.val(dropValue.trim());
        inputFeatureField.val(feature_id);
        inputCatField.val(category_id);
        inputListingField.val(listing_id);

        if (feature_id == null && category_id == null && listing_id == null && moreVal != null) {
    ...
    inputField.on('input', function() {
        ...
        var search_url = jQuery(inputField).data('search_url');
        ...
        } else if ((qString.length >= 3 && prevQString != qString) || (qString.length >= 3 && prevQuery < qString.length)) {
            ...
            jQuery.ajax({
                ...
                success: function(data) {
                    ...
                    if (data) {
                        ...
                        if (data.suggestions.tags || data.suggestions.tagsncats || data.suggestions.cats || data.suggestions.titles) {

                            if (data.suggestions.tags) {
                                jQuery.each(data.suggestions.tags, function(i, v) {
                                    resArray.push(v);
                                });
                            ...
                            if (data.suggestions.cats) {
                                ...
                                if (data.suggestions.tags == null && data.suggestions.tagsncats == null && data.suggestions.titles == null) {
                                    resArray = resArray;
                                } else {}
